...
public class R2CustomerController {
  ...
  //추가 .controller3
    ...
    public Mono<Customer> updateCustomer(@PathVariable Long id, @RequestBody Customer customer){
        Mono<Customer> customerMono = customerRepository.findById(id) .flatMap(existCust -> {
            existCust.setFirstName(customer.getFirstName()); existCust.setLastName(customer.getLastName()); return customerRepository.save(existCust);
        });
    ...
    public Mono<ResponseEntity<Void>> deleteCustomer(@PathVariable Long id) {
        return customerRepository.findById(id) .flatMap(existCust ->
                customerRepository.delete(existCust).then(Mono.just(ResponseEntity.ok().<Void>build())) ).defaultIfEmpty(ResponseEntity.notFound().build());
    }
